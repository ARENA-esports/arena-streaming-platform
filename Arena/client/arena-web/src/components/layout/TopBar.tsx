...
import { Search, User, Settings, LogOut, Moon, Sun, Video, BarChart2, Monitor } from 'lucide-react';
            ...
            {dropdownOpen && (
              <div className="absolute top-12 right-0 w-64 bg-arena-surface border border-arena-border rounded-md shadow-lg py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                ...
                <div className="py-1 border-b border-arena-border">
                  <Link to="#" className="flex items-center gap-3 px-4 py-2 hover:bg-arena-surfaceHover text-sm text-arena-text" onClick={() => setDropdownOpen(false)}>
                    <Video size={18} />
                    Channel
                  </Link>
                  <Link to="#" className="flex items-center gap-3 px-4 py-2 hover:bg-arena-surfaceHover text-sm text-arena-text" onClick={() => setDropdownOpen(false)}>
                ...
                {/* Settings & Theme */}
                <div className="py-1 border-b border-arena-border">
                  <Link to="/settings/profile" className="flex items-center gap-3 px-4 py-2 hover:bg-arena-surfaceHover text-sm text-arena-text" onClick={() => setDropdownOpen(false)}>
                    <Settings size={18} />
                    Settings
                  </Link>
                  <button 
                    className="w-full flex items-center justify-between px-4 py-2 hover:bg-arena-surfaceHover text-sm text-arena-text"
                    onClick={toggleTheme}
                  >
                    ...
                    <div className={`w-8 h-4 rounded-full p-0.5 transition-colors ${theme === 'dark' ? 'bg-purple-600' : theme === 'light' ? 'bg-arena-cyan' : 'bg-gray-500'}`}>
                ...
                <div className="py-1">
                  <button 
                    className="w-full flex items-center gap-3 px-4 py-2 hover:bg-arena-surfaceHover text-sm text-arena-text"
